...
class Ui(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.ui.pushButton_15.clicked.connect(self.showAllFilteredPeaks) #Shows all product peaks with min height filter
        self.ui.pushButton_16.clicked.connect(self.showFilteredStd) #Shows all internal std peaks with min height filter

        #Tab2
        self.ui.pushButton_19.clicked.connect(self.showDifferences) # shows sorted ranges list
        self.ui.pushButton_20.clicked.connect(self.showDifferencesTable) # shows all peaks with difference ranges column
        ...
        self.ui.pushButton_24.clicked.connect(self.calcTableandFAVSB) #Calculates the fractional area vs size before conc
        self.ui.pushButton_25.clicked.connect(self.showTableWithSize) #Shows table with polymer sizes in column
        self.ui.pushButton_23.clicked.connect(self.showFracAreaSizeBefore) #Shows fractional area vs size before conc fix
        ...
        self.ui.pushButton_42.clicked.connect(self.showPreliminaryDeleteGraph) #Shows the preliminary graph after polymer deletion
    ...
    def showAllProductPeaks(self):
        try:
            model = pandasModel(self.df_all_peaks.loc[self.df_all_peaks['Dye'] == 'B'])
            self.view = QTableView()
            self.view.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.Stretch)
            self.view.resizeColumnsToContents()
            self.view.setModel(model)
    ...
    def getMinHeight(self):
        ...
        try:
            ...
            self.df_all_filtered_peaks =  all_peaks_filtered(self.df_all_peaks, self.min_height) #Creates table with all product peaks above min height
...
#---------------------------------------------------------------------------------------------------------------
    ...
    def calcTableandFAVSB(self):
        ...
        try:
            ...
            combine_bounds = list(map(list, zip(low_bounds,upper_bounds))) # Returns: [[low_bound,upper_bound],[low_bound,upper_bound]]
