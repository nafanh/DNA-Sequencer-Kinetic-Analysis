...
def int_std_all(df,dye):
    df_int_all = df.loc[df['Dye'] == dye]
    return df_int_all

#Returns all product peaks filtered above certain height
def all_peaks_filtered(df,min_height,dye):
    df_hmin = df.loc[df['Height'].astype(int) > min_height]
    df_hmin_2 = df_hmin.loc[df_hmin['Dye'] == dye]
    return df_hmin_2

def all_peaks_filtered_global(df,min_height):
    df_hmin = df.loc[df['Height'].astype(int) > min_height]
    return df_hmin
def int_std_filtered(df,min_height_int_std,dye):
    df_int_std_filtered = df.loc[df['Height'].astype(int) > min_height_int_std]
    df_int_std_filtered_2 = df_int_std_filtered.loc[df_int_std_filtered['Dye'] == dye]
...
def sample_distance(filtered_data,int_dye,prod_dye):
    # gets peaks without internal standard
    df_no_int = filtered_data.loc[filtered_data['Dye'] == prod_dye]
    #print(df_no_int)

    # gets peaks with internal standard
    df_int_std = filtered_data.loc[filtered_data['Dye'] == int_dye]
